		...
		<el-main>
			<el-table :data="classList" style="width: 100%" stripe border>
				...
				<el-table-column label="教学老师" width="100" align="center">
					<template slot-scope="{row}">
						<span v-text="staff_teacher.find(item=>row.cls_stf_id_teacher===item.stf_id).stf_name "></span>
					</template>
				</el-table-column>
				<el-table-column label="教务老师" width="100" align="center">
					<template slot-scope="{row}">
						<span v-text="staff_admin.find(item=>row.cls_stf_id_admin===item.stf_id).stf_name "></span>
					</template>
				</el-table-column>
				<el-table-column label="就业老师" width="100" align="center">
					<template slot-scope="{row}">
						<span v-text="staff_job.find(item=>row.cls_stf_id_job===item.stf_id).stf_name"></span>
					</template>
				</el-table-column>
        ...
        export default {
                                        ...
                                        cls_name: [
                                                ...
                                                {min: 2, max: 20, message: '*员工名字长度在2 到 20个字符', trigger: 'blur'},
                                                {
                                                        validator: async(rule, value, callback) =>{
                                                                if(this.edit.model.old_name===value) callback();
                                                                if(await this.$http({
                ...
                methods: {
                        ...
                        beginUpdate(classMessage){
                                ...
                                this.edit.model.old_name= classMessage.cls_name;
	                                ...
	                                this.classList[i].cls_begin=begin;
                                        ...
                                        this.$message({
                                                ...
                                                if(typeof this.classNameValidateResult === "undefined"){
                                                        ...
                                                        let  formValidataField=["cls_dic_id_major","cls_stf_id_teacher","cls_stf_id_admin","cls_stf_id_job"]
                                                        await  Promise.all(formValidataField.map(item=>{
                ...
                created(){
                        let p1 = this.$http({url: "/staff/listbycategory/" + this.EnumStaffCategory.teacher});
                        let p2 = this.$http({url: "/staff/listbycategory/" + this.EnumStaffCategory.admin});
                        let p3 = this.$http({url: "/staff/listbycategory/" + this.EnumStaffCategory.job});
                        let p4 = this.$http({
                                url: "/class/list",
                                method: "post",
                                data: {

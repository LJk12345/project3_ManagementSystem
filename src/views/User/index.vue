<template>
	<el-container>
					...
					<el-table-column label="操作">
						<template slot-scope="{ row }">
							...
							<el-button size="small" type="warning" icon="el-icon-delete"
							           @click="Delect(row.user_name)"></el-button>
							<el-popover placement="right" >
								<el-button @click="beginRoleFuncConfig(row)" size="small" type="success" slot="reference">角色分配</el-button>
								<el-form inline>
		...
		<el-dialog :modal="false" :visible.sync="edit.isEdit" :title="edit.mode? '用户管理-新增' : '用户管理-修改'">
			<el-form ref="form" :model="edit.user" label-width="140px" :rules="rules"
			         @validate="formValidateHandler">
				<el-form-item label="用户名称" prop="user_name" v-show="edit.mode">
					<el-input placeholder="请输入用户名" v-model.trim="edit.user.user_name"></el-input>
				</el-form-item>
				<el-form-item label="密码" type="password" prop="user_pwd">
					<el-input placeholder="请输入用户密码" v-model="edit.user.user_pwd"></el-input>
				</el-form-item>
			</el-form>
        ...
        export default {
                ...
                methods: {
                        ...
                        async save(){
                                ...
                                //在吗能执行到这里，说明用户名和密码都通过了验证，可以执行后续真正的新增与修改操作了
                                if(this.edit.mode){
                                        await this.$http({
                                                url: "/user/add",
                                                method: "post",
                                        ...
                                        this.$message({
                                                message: `"${this.edit.user.user_name}"用户,添加成功!`,
                                                type: "success"
                                        });
                                }else{
                                        await this.$http({
	                ...
	                //分配
                        async roleFuncConfig(){
                                await this.$http({method: "post",url: "/user/config_role",data: this.edit.user})
